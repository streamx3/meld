    ...
    def _update_line_cache(self):
        ...
        for i, c in enumerate(self._merge_cache):
            ...
            for (diff, seq, lo, hi) in seq_params:
                ...
                if (start > last):
                    self._line_cache[seq][last:start] = [(None, prev[seq], next_chunks[seq])] * (start - last)
                ...
                self._line_cache[seq][start:end] = [(i, prev[seq], next_chunks[seq])] * (end - start)
                prev[seq], old_end[seq] = i, end

        for seq in range(3):
            last, end = old_end[seq], len(self._line_cache[seq])
            if (last < end):
                self._line_cache[seq][last:end] = [(None, prev[seq], next_chunks[seq])] * (end - last)
